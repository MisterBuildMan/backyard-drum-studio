# Master Plan

High-level overview of project phases.

## Phases

- [x] 0. Goals & Brief
  - Project motivation and intended use
  - Technical targets (60 dB isolation)
  - Timeline (end of 2026) and budget ($50k target, $70k cap)

- [-] 1. Planning & Permits
  - Site assessment
  - Local code research
  - Permit applications
  - Budget planning

- [-] 2. Design
  - 3D modeling (SketchUp)
  - Floor plan and dimensions
  - Sound isolation strategy
  - Electrical layout
  - Materials selection

- [ ] 3. Site Prep & Foundation
  - Ground clearing and leveling
  - Formwork
  - Concrete slab pour
  - Curing

- [ ] 4. Structure
  - Outer shell construction (CMU, wood frame, or hybrid — TBD)
  - Roof structure
  - Door and window rough openings

- [ ] 5. Exterior
  - Sheathing
  - Roofing
  - Siding
  - Exterior door and windows

- [ ] 6. Systems
  - Electrical rough-in
  - HVAC planning
  - Soundproofing layers

- [ ] 7. Interior Finish
  - Insulation
  - Drywall
  - Flooring
  - Interior door
  - Trim and paint

- [ ] 8. Acoustic Treatment
  - Bass traps
  - Absorption panels
  - Diffusion (if needed)

- [ ] 9. Final Setup
  - Equipment installation
  - Final inspection
  - Punch list completion

---

## Notes

- Phases may be split or combined as the project evolves
- Detailed specs are created just-in-time (1-2 phases ahead)
- See individual phase specs in `specs/` folder for details
